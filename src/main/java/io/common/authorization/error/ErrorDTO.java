package io.common.authorization.error;

import lombok.*;
import org.springframework.http.ResponseEntity;

import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;

@Getter
@ToString
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor
public class ErrorDTO {

    private String code;
    private String message;
    private String timestamp;

    public ErrorDTO(ErrorException e) {
        this.code = e.getCode();
        this.message = e.getMessage();
        this.timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
    }

    public static ErrorDTO BAD_REQUEST_ERROR() {
        return new ErrorDTO(ErrorCode.INVALID_FORMAT.getCode(), ErrorCode.INVALID_FORMAT.getMessage(), LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
    }

    public ResponseEntity<ErrorDTO> toResponse() {
        return ResponseEntity
                .status(ErrorCode.enumOf(this.code).getStatus())
                .body(this);
    }
}
